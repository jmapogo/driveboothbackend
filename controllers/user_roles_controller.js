module.exports = function (app, model, sequelize, Sequelize, ssCRUD, moment, mysql, underscore) {
	...
	var user_roles = model;
	var queryHelper;
	var primary_key = "";
	var all_columns = underscore.map(roles.rawAttributes, function (item) {
	...
	con.connect(function (err) {
		...
		con.query(query, function (err, result, fields) {
			...
			if (result.length > 0) {
				...
				var logController = new ssCRUD.Controller(roles);
				logController
				.addCreate(
				...
				app.get('/roles/:id', logController.getItem);
				app.get('/roles/', logController.getItems);
